...
This path never creates an officer Persona, copies an organization address onto a person, or approves a canonical claim. LinkedIn and Google Maps are not governed deterministic adapters because their platform terms do not support credential-free direct scraping and durable extraction for this product. A failure of either the research turn or the structured extraction degrades only this optional source; deterministic website, registry, Wikidata, and DNS results still finalize.
