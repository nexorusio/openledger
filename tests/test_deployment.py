...
def test_caddy_uses_application_login_instead_of_browser_basic_auth():
    ...
    assert "image: ${OPENLEDGER_RELEASE_IMAGE:?" in compose
...
def test_update_verifies_database_backup_before_migration():
    ...
    deploy_position = update_script.index("compose run --rm --no-deps migrate")
    assert backup_position < verification_position < deploy_position


def test_updater_restores_existing_secrets_instead_of_replacing_them():
    script = (REPOSITORY_ROOT / "deploy/update.sh").read_text()
    assert "Restore ${required} before deployment" in script
