"""Add mandatory P2 end-to-end pipeline lineage, curation, QC and research.

Revision ID: e2e1a7c9d401
Revises: b3e9d7c4a610

Downgrade is deliberately refused if any pipeline record exists: an application
rollback keeps this additive schema and must preserve submitted evidence.
"""

from alembic import op
from sqlalchemy import MetaData, Table, Column, String, select, func
# Frozen at e2e1: future runtime tables must never appear in this revision.
...
def _tables():
    metadata = MetaData()
    for name in ("cases", "personas", "investigation_jobs"):
        Table(name, metadata, Column("id", String(36), primary_key=True))
    register_pipeline_schema(metadata)
    return [
        table for table in metadata.sorted_tables if table.name.startswith("pipeline_")
    ]
...
def downgrade():
    ...
    for table in tables:
        if connection.scalar(select(func.count()).select_from(table)):
            raise RuntimeError(
                "Pipeline records exist; preserve expanded schema during code rollback"
            )
